...
class BDT(ShortRate):
    def __init__(self,x_,y_,vol_,arg=None):
    ...
    def calibrate(self):
        ...
        if len(self.x) != len(self.y) or len(self.x) != len(self.vol)+1:
            ...
            self.priceTree=np.triu(np.ones((len(self.x)+1,len(self.x)+1)))
            for i in range(len(self.shortRate)):
                ...
                if i == 0:
                    ...
                    res=minimize(BDT.SSE,[0.01,0.1],args=(self.shortRate,self.priceTree,deltaTime,i,zeroPrice[i],self.vol[i-1]))
                    if res.success:
                        self.shortRate[0:i+1,i]=[ res.x[0]*np.exp(2*res.x[1])**k for k in range(i+1) ]
                    else:
    ...
    def SSE(x,shortRateTree,priceTree,deltaTime,timeIndex,targetPrice,targetVol):
        ...
        r,sigma=x[0],x[1]
        shortRateTree[0:timeIndex+1,timeIndex]=[ r*np.exp(2*sigma)**k for k in range(timeIndex+1) ]
        for t in range(timeIndex,-1,-1):
            for j in range(0,t+1):
                priceTree[j,t]=0.5*(priceTree[j,t+1]+priceTree[j+1,t+1])*np.exp(-shortRateTree[j,t]*deltaTime[t])
