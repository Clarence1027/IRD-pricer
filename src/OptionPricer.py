...
class TreeBasedBondOptionPricer():
    def __init__(self,optStrike,optTTM,bondCpn,bondTTM,TTFirstPmt,bondFV=1,optCall=True,shortRateModel=None):
        ...
        if optTTM > bondTTM or TTFirstPmt < optTTM or TTFirstPmt > bondTTM:
            raise Exception('invalid option TTM with regard to bond TTM and TTFirstPmt')
        ...
        if shortRateModel is not None and ( shortRateModel.step > 0.5 or 0.5/shortRateModel.step != int(0.5/shortRateModel.step) ):
            raise Exception('improper short rate model time step. It MUST be less or equal than 0.5 and is a divider of 0.5')
        if shortRateModel is not None and optTTM/shortRateModel.step != int(optTTM/shortRateModel.step):
            raise Exception('improper short rate model time step for option TTM')
    ...
    def _getPmtTimeIndex(self):
        firstPmtTimeIndex = self._getOptMaturityTimeIndex() + int((self.TTFirstPmt-self.optTTM)/self.model.step)
    ...
    def optPayOffAtMaturity(self):
        ...
        # remember finally reshape to two dimensional for later discount convenience
        ...
        for t in pmtTimeIndx:
            payoff += (self.model.statePrice[t][:optMatTimeIndx+1,optMatTimeIndx] * self.bondCpn * self.bondFV)

        payoff += (self.model.statePrice[t][:optMatTimeIndx+1, optMatTimeIndx] * self.bondFV)
    ...
    def price(self,showAllStates=False):
        ...
        for t in range(optMatTimeIndx-1,-1,-1):
            for j in range(0,t+1):
                optPrice[j,t]=0.5*(optPrice[j,t+1]+optPrice[j+1,t+1])*np.exp(-self.model.shortRate[j,t]*self.model.step)
...
def demo():
    ...
    pricer=TreeBasedBondOptionPricer(optStrike,optTTM,bondCpn,bondTTM,TTFirstPmt,shortRateModel=bdt)
